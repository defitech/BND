...
class Defitech_Controller {
    ...
    protected function getBookList() {
        ...
        $books = array();
        $types = Defitech_Book_Type::getListToArray();
        $editors = Defitech_Book_Editor::getListToArray();
        foreach ($rows as $row) {
            $niveaux = $row->findManyToManyRowset('Defitech_Niveau', 'Defitech_Book_Niveau');
            $ns = array();
    ...
    protected function getBook() {
        ...
        $niveaux_all = Defitech_Niveau::getListToArray();
        // récupération des niveaux liés au livre
        $table_link = new Defitech_Book_Niveau();
        $niveaux_tmp = $book->findManyToManyRowset('Defitech_Niveau', 'Defitech_Book_Niveau');
    ...
    protected function saveBook() {
        ...
        $config = Defitech_Config::getInstance();
        ...
        if ($thumb['error'] == UPLOAD_ERR_OK) {
            if (in_array($thumb['type'], $valid_extensions)) {
                $img = Defitech_Util::getSlug($row->title) . strrchr($thumb['name'], '.');
                move_uploaded_file($thumb['tmp_name'], Defitech_Book::getThumbPath(true) . $img);
                // set du nouveau thumb
                $row->thumb = Defitech_Book::getThumbFolder() . $img;
            } else {
                $success = false;
        ...
        if ($pdf['error'] == UPLOAD_ERR_OK) {
            if ($pdf['type'] == 'application/pdf') {
                ...
                $p = Defitech_Util::getSlug($row->title) . '.pdf';
                $i = Defitech_Util::getSlug($row->title) . '.jpg';
                move_uploaded_file($pdf['tmp_name'], $path . $p);
                $output = $this->generatePdfFirstPageThumb($path . $p, Defitech_Book::getThumbPath(true). $i);
                $log[] = $output;
                // set du nouveau pdf et de son thumb
                $row->thumb = Defitech_Book::getThumbFolder() . $i;
    ...
    protected function removeBook() {
        Defitech_Config::getInstance()->testIssetAuser();
    ...
    private function generatePdfFirstPageThumb($pdf, $img) {
        $path_convert = Defitech_Config::getInstance()->getData()->path->convert;
    ...
    protected function getTypeList() {
        $rowset = Defitech_Book_Type::getList();
        $data = array();
    ...
    protected function getEditorList() {
        $rowset = Defitech_Book_Editor::getList();
        $data = array();
        foreach ($rowset as $row) {
            $data[] = array(
                'id' => $row->id,
    ...
    protected function addEditor() {
        $table = new Defitech_Book_Editor();
        $table->insert(array(
    ...
    protected function getNiveauList() {
        $rowset = Defitech_Niveau::getList();
        $data = array();
        foreach ($rowset as $row) {
            $data[] = array(
                'id' => $row->id,
    ...
    protected function import() {
        ...
        if ($file['error'] == UPLOAD_ERR_OK && $file['type'] == 'text/csv' && ($handle = fopen($file['tmp_name'], 'r')) !== false) {
            ...
            $lines = 0;
            $continue = true;
            $this->importTypes = Defitech_Book_Type::getListToArray();
            $this->importEditors = Defitech_Book_Editor::getListToArray();
            $this->importNiveaux = Defitech_Niveau::getListToArray();
    ...
    private function makeDataFromImportLine($line) {
        ...
        return array_merge($csv, array(
            'filename' => $filepdf,
            'thumb' => $file ? Defitech_Book::getThumbPath() . $fileimage : null,
            'pathimg' => Defitech_Book::getThumbPath(true) . $fileimage,
            'pathpdf' => Defitech_Config::getInstance()->getData()->path->pdf . $filepdf,
            'tags' => Defitech_Util::getSlug($csv['titre'], ',', 2),
    ...
    private function importGetEditor($csv) {
        ...
        $slug = Defitech_Util::getSlug($csv['editeur']);
        $id = null;
        foreach ($this->importEditors as $ide => $val) {
            if ($slug == Defitech_Util::getSlug($val)) {
                $id = $ide;
                break;
            }
        }
    ...
    private function importGetType($csv) {
        ...
        $slug = Defitech_Util::getSlug($csv['matiere']);
        $id = null;
    ...
    private function importGetNiveaux($csv) {
        ...
        foreach ($niveaux as $niveau) {
            ...
            $slug = Defitech_Util::getSlug($niveau);
            $id = null;
